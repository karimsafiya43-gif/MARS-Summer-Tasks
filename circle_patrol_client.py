...
class CircleClient(Node):
    ...
    def get_feedback(self, fback_msg):
        ...
        self.get_logger().info(f"{fb.final_report}")

    def get_result_callback(self, future):
        result = future.result().result
        self.get_logger().info(f"final status: {result.current_status}")
        self.get_logger().info(f"distance travelled: {result.distance_travelled:.2f}")
